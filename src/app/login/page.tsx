...
export default function Login() {
  const [step, setStep] = useState<"LOGIN" | "OTP" | "BACKUP_CODE">("LOGIN");
  ...
  const handlePasskeyLogin = async () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        showToast("An unknown error occurred during Passkey login.", "error");
    } finally {
            ...
            <AnimatePresence mode="wait">
              ...
              {step === "OTP" && (
                ...
                >
                  ...
                  <p className="text-slate-500 text-sm mb-10 max-w-sm">
                    Please open your Authenticator app (e.g., Google
                    Authenticator or Authy) to find the code for{" "}
              ...
              {step === "BACKUP_CODE" && (
                ...
                >
                  ...
                  <p className="text-slate-500 text-sm mb-10 max-w-sm">
                    Enter one of your 8-character backup codes. Each code can
                    only be used once.
                  </p>
                  ...
                  <div className="flex items-center justify-center gap-1.5 sm:gap-2 mb-8">
                    <div className="flex gap-1.5 sm:gap-2">
                      {backupCode.slice(0, 4).map((digit, idx) => (
                        <input
                          ...
                          onChange={(e) =>
                            handleChangeBackup(idx, e.target.value)
                          }
                    ...
                    <span className="text-slate-300 font-bold text-2xl pb-1">
                      -
                    </span>
                      ...
                      {backupCode.slice(4, 8).map((digit, idx) => {
                        ...
                        return (
                          <input
                            ...
                            onKeyDown={(e) =>
                              handleKeyDownBackup(actualIndex, e)
                            }
                            className={`w-9 h-12 sm:w-11 sm:h-14 text-center text-xl font-bold rounded-xl border-2 transition-all focus:outline-none disabled:opacity-50 uppercase ${
                              digit
                                ? "bg-white border-teal-500 text-teal-600 shadow-sm shadow-teal-500/20"
